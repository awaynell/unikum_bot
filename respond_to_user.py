...
async def handle_model_response(temp_reply, chat_id, message_id, dialog_history, context, update, user_message, sent_message, context_history_key):
        ...
        async with await loop.run_in_executor(None, lambda: session.post(api_url, json=payload)) as response:
            if response.status == 200:
                ...
                try:
                    ...
                    await context.bot.edit_message_text(chat_id=chat_id, message_id=sent_message.message_id, text=f"{temp_reply} \n\n `Провайдер {provider}, модель {model}`", parse_mode='Markdown')
